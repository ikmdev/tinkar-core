 ...
 * <p>Services implementing this interface will have their startup and shutdown
 * methods invoked by the ServiceLifecycleManager in phase order.
 * <p>Services are discovered once during application initialization and cannot
 ...
 * }</pre>
 *
 * <p><b>Mutual Exclusion Example</b></p>
 * <pre>{@code
 * @LifecyclePhase(ServiceLifecyclePhase.DATA_STORAGE)
 ...
 * }</pre>
 *
 * <p><b>Static Discovery Facade</b></p>
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p><b>Phase Override via Command Line</b></p>
 * <pre>
...
public interface ServiceLifecycle {
     ...
     * <p>     * Services should initialize resources, establish connections, and prepare
     * for operation. This method should be fast and non-blocking where possible.
     * <p>     * If this method throws an exception, the entire application startup will
     ...
     * <p>     * Services should release resources, close connections, flush buffers,
     * and perform cleanup. This method must not throw exceptions - any errors
     * should be logged internally.
     * <p>     * All services will be shut down even if some fail. Exceptions are logged
     ...
     * <p>     * This method is used if no {@link LifecyclePhase} annotation is present.
     * The annotation takes precedence if both are specified.
     * <p>     * Can be overridden via system property:
     ...
     * <p>     * Services with lower sub-priority values start first within their phase.
     * Valid range is 0-99, with 50 as the default.
     * <p>     * Most services should use the default. Only specify a sub-priority if
     ...
     * <p>     * This method is called AFTER discovery but BEFORE startup. If it returns false,
     ...
     * <p>     * Services returning the same non-null group will be treated as mutually exclusive -
     ...
     * <p>     * This method allows other services to discover and access services managed by
     ...
     * <p>     * <b>Note:</b> For services that extend {@link ProviderController}, discovery should
     ...
     * <p>     * Most services don't provide anything to others and should return {@code Optional.empty()}.
