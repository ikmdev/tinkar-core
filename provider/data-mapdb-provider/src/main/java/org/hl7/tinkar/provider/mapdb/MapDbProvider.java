package org.hl7.tinkar.provider.mapdb;
...
public class MapDbProvider implements PrimitiveDataService {
    ...
    public MapDbProvider() {
        File configuredFile = ServiceProperties.get(ServiceKeys.PRIMITIVE_DATA_LOCATION, mapdbFile);
        configuredFile.getParentFile().mkdirs();
        System.out.println("Starting MapDbProvider from: " + configuredFile.getAbsolutePath());
        this.db = DBMaker.fileDB(configuredFile).fileMmapEnable().make();
