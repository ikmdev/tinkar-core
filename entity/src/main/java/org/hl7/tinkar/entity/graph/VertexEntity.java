...
public class VertexEntity implements Vertex, VertexId {
    ...
    public static <T extends Object> T abstractObject(Object object) {
        if (object instanceof Concept) {
            if (object instanceof ConceptProxy) {
             ...
             return (T) ConceptProxy.make(Get.entityService().nidForPublicId((Concept) object));
        } else if (object instanceof Semantic) {
            if (object instanceof SemanticProxy) {
                return (T) object;
            }
            return (T) SemanticProxy.make(Get.entityService().nidForPublicId((Semantic) object));
        } else if (object instanceof PatternForSemantic) {
            if (object instanceof PatternForSemanticProxy) {
                return (T) object;
            }
            return (T) PatternForSemanticProxy.make(Get.entityService().nidForPublicId((PatternForSemantic) object));
            ...
            return (T) StampDTO.builder()
                    ...
                    .statusPublicId(stampValue.status().publicId())
    ...
    private void fill(Vertex another) {
        ...
        if (another.meaning() instanceof ConceptEntity) {
            ...
            this.meaningNid = Get.entityService().nidForPublicId(another.meaning());
        }
        ...
        another.propertyKeys().forEach(concept -> {
            mutableProperties.put(Get.entityService().nidForPublicId(concept), abstractObject(another.propertyFast(concept)));
        });
        this.properties = mutableProperties.toImmutable();
    }
    ...
    public <T> T propertyFast(Concept propertyConcept) {
        ...
        return (T) properties.get(Get.entityService().nidForPublicId(propertyConcept));
    }
    ...
    public static VertexEntity make(Vertex vertex) {
        VertexEntity vertexEntity = new VertexEntity();
        vertexEntity.fill(vertex);
        return vertexEntity;
    }

    public static VertexEntity make(ByteBuf readBuf, byte entityFormatVersion) {
        if (entityFormatVersion == ENTITY_FORMAT_VERSION) {
            VertexEntity vertexEntity = new VertexEntity();
            vertexEntity.fill(readBuf, entityFormatVersion);
            return vertexEntity;
        } else {
