...
public class LoadEntitiesFromDTO {
    ...
    public Integer call() throws IOException {
        try {
            ...
            int[] originNids = Get.entityService().entityNidsOfPattern(PATH_ORIGINS_PATTERN.nid());
            ...
            int[] referencingSemanticDescriptions = Get.entityService().semanticNidsForComponentOfPattern(PATH_ORIGINS_PATTERN.nid(), DESCRIPTION_PATTERN.nid());
            for (int descriptionNid: referencingSemanticDescriptions) {
                LOG.info("Semantic of pattern DESCRIPTION_PATTERN referencing PATH_ORIGINS_PATTERN: \n    " + Get.entityService().getEntityFast(descriptionNid));
                for (int acceptibilityNid: Get.entityService().semanticNidsForComponentOfPattern(descriptionNid, TinkarTerm.US_ENGLISH_DIALECT.nid())) {
                    LOG.info("  Acceptability US: \n    " + Get.entityService().getEntityFast(acceptibilityNid));
                }
                for (int acceptibilityNid: Get.entityService().semanticNidsForComponentOfPattern(descriptionNid, TinkarTerm.GB_ENGLISH_DIALECT.nid())) {
