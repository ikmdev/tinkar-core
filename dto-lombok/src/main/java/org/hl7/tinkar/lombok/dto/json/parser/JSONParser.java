...
public class JSONParser {
    ...
    public Object parse(Reader in, ContainerFactory containerFactory) throws ParseException {
        ...
        LinkedList<Object> valueStack = new LinkedList<>();
            do {
                ...
                switch (status) {
                    case S_INIT:
                        handleS_INIT(containerFactory, statusStack, valueStack);
                        break;
                    ...
                    case S_IN_OBJECT:
                        handleS_IN_OBJECT(statusStack, valueStack);
                        break;

                    case S_PASSED_PAIR_KEY:
                        handleS_PASSED_PAIR_KEY(containerFactory, statusStack, valueStack);
                        break;

                    case S_IN_ARRAY:
                        handleS_IN_ARRAY(containerFactory, statusStack, valueStack);
                        break;
                    ...
                    default:
                        throw new ParseException(getPosition(), ParseException.ErrorType.UNEXPECTED_EXCEPTION, token);
                }
    ...
    public void handleS_IN_ARRAY(ContainerFactory containerFactory, LinkedList<Integer> statusStack, LinkedList<Object> valueStack) {
        switch (token.type) {
            ...
            case TYPE_VALUE: {
                ...
                if (token.value instanceof String) {
                    if (UuidUtil.isUUID((String) token.value)) {
                        val.add(UUID.fromString((String) token.value));
                    } else if (InstantUtil.parse((String) token.value).isPresent()) {
                        val.add(Instant.parse((String) token.value));
                    } else {
                        val.add(token.value);
                    }
    ...
    public boolean handleParse(ContentHandler contentHandler, LinkedList<Integer> statusStack) throws ParseException {
        switch (status) {
            case S_INIT: {
                if (handleS_INIT(contentHandler, statusStack))
                    return true;
            }
            break;
